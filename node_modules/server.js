...
app.get('/api/movie-horror', async (req, res) => {
    try {
      const url = 'https://www.mattfishwick.net/genre/horror/';
      const { data } = await axios.get(url);
      const $ = load(data);
...
app.get('/api/movie-rating', async (req, res) => {
  try {
      const urls = [
          'https://www.mattfishwick.net/?s=&search=advanced&post_type=movie&index=&orderby=rating&genre=&movieyear=&country=&quality=',
          'https://www.mattfishwick.net/page/2/?s&search=advanced&post_type=movie&index&orderby=rating&genre&movieyear&country&quality#038;search=advanced&post_type=movie&index&orderby=rating&genre&movieyear&country&quality',
          'https://www.mattfishwick.net/?s=&search=advanced&post_type=movie&index=&orderby=rating&genre=&movieyear=&country=&quality=',
          'https://www.mattfishwick.net/page/3/?s&search=advanced&post_type=movie&index&orderby=rating&genre&movieyear&country&quality#038;search=advanced&post_type=movie&index&orderby=rating&genre&movieyear&country&quality',
          'https://www.mattfishwick.net/page/4/?s&search=advanced&post_type=movie&index&orderby=rating&genre&movieyear&country&quality#038;search=advanced&post_type=movie&index&orderby=rating&genre&movieyear&country&quality',
      ...
      for (const url of urls) {
          ...
          $('article.item-infinite').each((index, element) => {
              ...
              // Pengecekan untuk menghindari link yang mengandung '/tv/'
              if (link && !link.includes('/tv/')) {
                  const imageUrl = $(element).find('img').attr('srcset') || $(element).find('img').attr('src');
                  const image = imageUrl ? (() => {
                      const srcsetArray = imageUrl.split(',').map(item => item.trim());
                      return srcsetArray[srcsetArray.length - 1].split(' ')[0];
                  })() : null;
...
app.get('/api/movie-indo', async (req, res) => {
  try {
      const urls = [
          'https://www.mattfishwick.net/genre/indonesia/',
          'https://www.mattfishwick.net/genre/indonesia/page/2',
          'https://www.mattfishwick.net/genre/indonesia/page/3',
          'https://www.mattfishwick.net/genre/indonesia/page/4',
          'https://www.mattfishwick.net/genre/indonesia/page/5',
